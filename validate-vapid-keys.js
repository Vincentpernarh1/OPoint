...
// Verify key lengths (87-88 chars is valid for public key due to base64url encoding)
console.log('📏 Checking key lengths...');
const publicKeyValid = publicKey.length >= 87 && publicKey.length <= 88;
console.log('   Public key length:', publicKey.length, publicKeyValid ? '✅' : '⚠️ (expected 87-88)');
...
const publicKeyValid = publicKey.length >= 87 && publicKey.length <= 88;
if (publicKeyValid && privateKey.length === 43 && publicKey.startsWith('B')) {
