...
import { TrendingUpIcon, TrendingDownIcon, LogoIcon, ChevronDownIcon, ArrowLeftIcon, SmartphoneIcon, SearchIcon } from "../Icons/Icons";
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                    {filteredEmployees.length > 0 ? (
                        filteredEmployees.map(user => (
                            <button key={user.id} onClick={() => setSelectedEmployee(user)} className="group bg-white p-4 border border-gray-200 rounded-2xl text-center hover:shadow-xl hover:border-primary/50 transition-all duration-300 transform hover:-translate-y-1">
                                {user.avatarUrl ? (
                                    ...
                                    <div className="w-16 h-16 rounded-full mx-auto mb-3 bg-gradient-to-r from-primary to-purple-600 text-white flex items-center justify-center text-lg font-semibold shadow-md group-hover:shadow-lg transition-all">
